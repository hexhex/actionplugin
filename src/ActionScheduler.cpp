...
void ActionScheduler::executionModeController(std::multimap<int, Tuple>& multimapOfExecution) {
...
//    std::cerr << std::endl;
    ...
    if (tuple[precedence_position].isIntegerTerm() && id_actionOption.isConstantTerm()) {

      precedence = tuple[precedence_position].address;

      std::cerr << "actionOption: " << registryPtr->getTermStringByID(id_actionOption) << std::endl;
      ...
      if (id_actionOption == ctxData.id_brave)
        ...
        if (!isPresentInAllAnswerset(action_tuple)) {
          std::cerr << "This action atom isn't present in all AnswerSet: ";
          ...
          if (!isPresentInAllTheBestModelsAnswerset(action_tuple)) {
            std::cerr << "This action atom isn't present in all BestModels AnswerSet: ";
            ActionPlugin::printTuple(action_tuple, registryPtr);
            continue;
          } else
            throw PluginError("Error in the selection of brave, cautious or preferred_cautious");

      multimapOfExecution.insert(std::pair<int, Tuple>(precedence, action_tuple));

    } else
      throw PluginError("Precedence isn't Integer term or ??? haven't a valid value");
...
bool ActionScheduler::isPresentInAllTheBestModelsAnswerset(const Tuple& action_tuple) {
...
bool ActionScheduler::thisAnswerSetContainsThisAction(const AnswerSetPtr& answerSetPtr,
...
void ActionScheduler::executionModeRewriter(const std::multimap<int, Tuple>& multimapOfExecution,
...
bool ActionScheduler::checkIfTheListIsCorrect(const std::multimap<int, Tuple>& multimapOfExecution,
