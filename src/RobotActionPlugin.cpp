...
void RobotActionPlugin::RobotActionAtom::retrieve(const Environment& environment, const Query& query, Answer& answer) {
...
void RobotActionPlugin::RobotAction::execute(Environment& environment, const Registry& registry, const Tuple& parms,
...
std::vector<PluginAtomPtr> RobotActionPlugin::createAtoms(ProgramCtx& ctx) const {
	std::vector < PluginAtomPtr > ret;
	ret.push_back(PluginAtomPtr(new RobotActionAtom, PluginPtrDeleter<PluginAtom>()));
	return ret;
}

std::vector<PluginActionBasePtr> RobotActionPlugin::createActions(ProgramCtx& ctx) const {
	std::vector<PluginActionBasePtr> ret;
	ret.push_back(
			PluginActionBasePtr(new RobotAction, PluginPtrDeleter<PluginActionBase>()));
	return ret;
}
