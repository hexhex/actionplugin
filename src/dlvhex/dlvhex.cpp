...
printUsage(std::ostream &out, bool full)
{
	...
	out << "     --               Parse from stdin." << std::endl
		...
		<< " -o, --noshowactions  Don't display action atoms in the answer sets" << std::endl
		<< " -e, --executeactions Execute the actions in according to the schema" << std::endl;
}
...
main (int argc, char *argv[])
{
  ...
  Globals::Instance()->setOption("NoShowActions",0);
  Globals::Instance()->setOption("ExecuteActions",0);
  ...
  static struct option longopts[] =
    {
	  ...
	  { "noshowactions", no_argument, 0, 'o' },
	  { "executeactions", no_argument, 0, 'e' },
      { NULL, 0, NULL, 0 }
    };
  ...
  while ((ch = getopt_long(argc, argv, shortopts, longopts, NULL)) != -1)
	...
	case 'o':
	  Globals::Instance()->setOption("NoShowActions",1);
	  break;

	case 'e':
	  Globals::Instance()->setOption("ExecuteActions",1);
	  break;
  ...
  try
      ...
      if (optionKeepNSPrefix)
	{
      ...
      pctx.output();
	  if(Globals::Instance()->getOption("ExecuteActions"))
      {
	  	pctx.evaluateActions();
	  }
