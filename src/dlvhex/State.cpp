...
void
State::createSchema(ProgramCtx*)
{ }

void 
State::evaluateActions(ProgramCtx*)
{ }
...
PostProcessState::postProcess(ProgramCtx* ctx)
{
  ...
  ctx->getResultContainer()->recoverActions();
...
OutputState::output(ProgramCtx* ctx)
{
  ...
  boost::shared_ptr<State> next(new ActionState);
  changeState(ctx, next);
...
void 
ActionState::createSchema(ProgramCtx* ctx)
{
	DEBUG_START_TIMER;
	
	sorted_full_intersection=findIntersection(ctx->getResultContainer());
	ResultContainer* rc=new ResultContainer();
	ResultContainer::result_t::iterator ri=ctx->getResultContainer()->getAnswerSets().begin();
	
	std::vector<ActionAtom*> candidate_schema;

	if (!((**ri).hasWeights()))
	{
		bestmodel=*ri;
		ResultContainer* newcontainer=new ResultContainer();
		newcontainer->addSet(*bestmodel);
		candidate_schema=findIntersection(newcontainer);
		sorted_best_intersection=sorted_full_intersection;
		delete newcontainer;
	}
	else
	{
		...
		bestmodel=*ri;
		ri++;
		for(;ri!=ctx->getResultContainer()->getAnswerSets().end();++ri)
		{
			if((**ri).compareWeights(**((rc->getAnswerSets()).begin()))==-1)
			{
				rc->makeEmpty();				
				rc->addSet(**ri);
				bestmodel=*ri;
			}
			else if((**ri).compareWeights(**((rc->getAnswerSets()).begin()))==0)
			{
				rc->addSet(**ri);
			}			
		}	
		...
		candidate_schema=findIntersection(newcontainer);
		delete newcontainer;
		sorted_best_intersection=findIntersection(rc);
	}


	RawPrintVisitor rpv(std::cerr);

	std::cout << "The selected best model is:\n";
	(*bestmodel).accept(rpv);

	for(std::vector<ActionAtom*>::iterator it=candidate_schema.begin(); it!=candidate_schema.end(); ++it)
	{	
		if((*it)->getOption()=="b")
			schema.push_back(*it);

		else if((*it)->getOption()=="c")
		{			
			for(std::vector<ActionAtom*>::iterator ai=sorted_full_intersection.begin();
				((ai!=sorted_full_intersection.end()) && ((**ai).getPrecedence().getInt()<=(*it)->getPrecedence().getInt()));
				++ai)
			{				
				if((**ai).isEqualSyntax(**it))
				{				
					schema.push_back(*it);
					break;
				}
		...
		else if((*it)->getOption()=="cp")
		{
			for(std::vector<ActionAtom*>::iterator ai=sorted_best_intersection.begin();
			((ai!=sorted_best_intersection.end()) && ((**ai).getPrecedence().getInt()<=(*it)->getPrecedence().getInt()));
			++ai)
			{
				if((**ai).isEqualSyntax(**it))
				{
	...
	evaluateActions(ctx);
 	boost::shared_ptr<State> next(new State);
  	changeState(ctx, next);
  	//                123456789-123456789-123456789-123456789-123456789-123456789-123456789-123456789-
  	DEBUG_STOP_TIMER("Creating Schema:                        ");
}

std::vector<ActionAtom*>
ActionState::findIntersection(ResultContainer *rc)
{
	std::vector<ActionAtom*> inter_set;	
	ResultContainer::result_t::iterator hold=rc->getAnswerSets().begin();
	int count=1;
	int total=rc->getAnswerSets().size();
	if(total==1) 
	{
		for(AtomSet::const_iterator si=(**hold).begin();si!=(**hold).end();++si)
		{		
			if(typeid(*si)==typeid(ActionAtom))
			{
				ActionAtom * aaPtr=const_cast<ActionAtom*>(static_cast<const ActionAtom*>(&(*si)));			
				inter_set.push_back(aaPtr);
            }
        }
	}
	else
	{	
		...
		{	
			...
			{			
				std::cout << std::endl;			
				ResultContainer::result_t::iterator save=hold;			
				for(ResultContainer::result_t::iterator ri=++save;
					ri!=rc->getAnswerSets().end();
					++ri)
				{
	
					AtomSet::const_iterator pi;
					for(pi=(**ri).begin();pi!=(**ri).end();++pi)
					{
						if(typeid(*pi)==typeid(ActionAtom))
						{	
							const ActionAtom* aa1=static_cast<const ActionAtom*>(&(*pi));						
				        	const ActionAtom* aa2=static_cast<const ActionAtom*>(&(*si));
							if((*aa1).isEqualSyntax(*aa2))			
					    	{						
								count++;	
								break;
							}
	...
	std::sort(inter_set.begin(),inter_set.end(),comparatorObject);
	return inter_set;
...
void
ActionState::evaluateActions(ProgramCtx* ctx)
{
  DEBUG_START_TIMER;

  PluginContainer* pluginContainer=ctx->getPluginContainer();
  std::cerr << "\n\nPlease press ENTER to execute the actions";
  getchar();
  for(std::vector<ActionAtom*>::iterator ai=schema.begin();ai!=schema.end();++ai)
  {
  		std::string fnc=(*ai)->getFunctionName();
		boost::shared_ptr<PluginAtom> pluginAtom=	pluginContainer->getAtom(fnc);
		if(!pluginAtom)
		{
			throw PluginError("Could not find plugin for action atom #" + fnc);
		}
		AtomSet inputSet;
		const std::vector<PluginAtom::InputType>& inputtypes = pluginAtom->getInputTypes();
		Tuple::const_iterator termit=(*ai)->getInputTerms().begin();
	    for(std::vector<PluginAtom::InputType>::const_iterator typeit=inputtypes.begin();
			typeit!=inputtypes.end() && termit!=(*ai)->getInputTerms().end();++typeit,++termit)
		{
			switch(*typeit)
			{
				...
				case PluginAtom::PREDICATE:
					bestmodel->matchPredicate(termit->getString(), inputSet);
					break;
				default:
					break;
			}
		...
		Tuple dummy;
		PluginAtom::Query query(inputSet,(*ai)->getInputTerms(),dummy);
  ...
  //                123456789-123456789-123456789-123456789-123456789-123456789-123456789-123456789-
  DEBUG_STOP_TIMER("Evaluating Actions:                        ");
}
