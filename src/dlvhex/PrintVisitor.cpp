...
void
RawPrintVisitor::visit(ActionAtom* const aa)
{
  stream << '#' << aa->getFunctionName();

  const Tuple& inputList = aa->getInputTerms();
  ...
  const std::string c=aa->getOption();
  stream << c;
  if(aa->getPrecedence().isString() && aa->getPrecedence().getString()!="0")
  {
  	stream << ",";

  	const Term& precedence = aa->getPrecedence();

        stream << precedence.getString();
  }
  else if(aa->getPrecedence().isInt() && aa->getPrecedence().getInt()!=0)
  {
        stream << ",";

  	const Term& precedence = aa->getPrecedence();

        stream << precedence.getInt();
  }
...
void
DLVPrintVisitor::visit(ActionAtom* const aa)
{
  //
  // the replacement atom contains both the input and the output list
  //
  ...
  //replacementArgs.insert(replacementArgs.end(), tup.begin(), tup.end());
  replacementArgs.push_back(Term(aa->getOption()));
  const Term& precedence = aa->getPrecedence();

  replacementArgs.push_back(precedence);

  Atom tmp(aa->getReplacementName(), replacementArgs);
