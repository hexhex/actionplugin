...
//			std::cout<<"new as has weight:"<<new_as.getWeight(s)<<" at level "<<s<<"\n";
	...
	ActionAddonContainer* container = new ActionAddonContainer("");
	ActionAddonInterface::ActionAddonAtomFunctionMap a_fm; 
	std::vector<ActionAddonInterface*> addons = container->importActionAddons();
...
//			//std::cout << "an action atom is to be executed: " << (**vit).getPredicate().getString()<<std::endl;//execute action
		      bool executed=false;
		      for (std::vector<ActionAddonInterface*>::const_iterator pi = addons.begin();
				   pi != addons.end() && !executed; ++pi)
				{
	//std::cout << "inside for"<<std::endl;					
					(*pi)->getAtoms(a_fm);
					for(ActionAddonInterface::ActionAddonAtomFunctionMap::iterator fm_it=a_fm.begin(); fm_it!=a_fm.end(); ++fm_it)
					{
		...
		else if(option=="c")
			{
				...
				if(Exists(*rc_intersection, (*vit)))
				{
					//std::cout << "an action atom is to be executed but not yet implemented: " << (**vit).getPredicate().getString();//execute action
					 bool executed=false;
		      			 for (std::vector<ActionAddonInterface*>::const_iterator pi = addons.begin();pi != addons.end() && !executed; ++pi)
						{
	//std::cout << "inside for"<<std::endl;					
					(*pi)->getAtoms(a_fm);
						for(ActionAddonInterface::ActionAddonAtomFunctionMap::iterator fm_it=a_fm.begin(); fm_it!=a_fm.end(); ++fm_it)
