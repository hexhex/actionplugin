 ...
 *
 * \brief Declaration of Classes AddonAtom, PluginRewriter,
 * and PluginInterface.
 */
 ...
 * First, we have to subclass AddonAtom to create our own \b RDFatom class:
 * 
 * \code
 *     class RDFatom : public AddonAtom
 *     {
 * \endcode
 * 
 ...
 * AddonAtom::addInputPredicate(). Another possibility would have been to define the
 ...
 * predicate. To define a constant input parameter, we use AddonAtom::addInputConstant().
 ...
 *         {
 *             boost::shared_ptr<AddonAtom> rdf(new RDFatom);
 *             a["rdf"] = rdf;
 *         }
 * \endcode
 * 
 ...
 * its AddonAtom::retrieve() function) in this map. Naturally, a plugin can
 * comprise several different Plugin-Atoms, which are all registered here:
 *
 * \code
 *             boost::shared_ptr<AddonAtom> str_cat(new strcat_atom);
 *             boost::shared_ptr<AddonAtom> str_find(new strfind_atom);
 *             boost::shared_ptr<AddonAtom> str_erase(new strerase_atom);
 *             a["str_cat"] = str_cat;
 *             a["str_find"] = str_find;
 ...
 *
 * An external atom is represented by a subclassed AddonAtom. The actual
 * evaluation function of the atom is realized in the AddonAtom::retrieve()
 * method. In the constructor, the user must specify the types of input terms
 * and the output arity of the atom.
 */
class DLVHEX_EXPORT AddonAtom
{
public:
 ...
 *
 * \ingroup addonframework
 *
 * \section intro Overview
 *
 * The AddonInterface class can be seen as a wrapper for all user-defined
 * features of a addon, which are:
 ...
 * .
 *
 * The user has to subclass from AddonInterface in order to implement a addon:
 *
 * \code
 * class MyShinyAddon : public AddonInterface
 * {
 *     ...
 * }
 * \endcode
 * 
 * Within this definition, the user might want to override some of the following
 * methods:
 * - getAtoms() \n
 *   At least, the addon will contain some external atoms, which
 *   are implemented by deriving custom classes from AddonAtom and then are
 *   incorporated by registering them in the getAtoms function.
 * - createConverter(), createRewriter() \n
 *   A addon can provide a \e Converter
 *   (see AddonConverter), which will receive the input program before dlvhex
 *   will start looking at it. With this facility, a addon can for example
 *   provide a conversion from a different language to a hex-program. After
 *   dlvhex has parsed the input program, it will pass it to those found addons
 *   that have provided a \e Rewriter (see AddonRewriter), which can alter the
 *   program to their liking, e.g., implement some syntactic sugar regarding
 *   their external atoms. In fact, a addon could also supply only a converter
 ...
 *   passes this graph to those addons that have implemented an \e Optimizer
 *   (see AddonOptimizer). Such an optimizer can then modify the graph
 *   directly, which is more intuitive in case of optimization tasks than to
 *   work on the textual program repesentation.
 * - setOptions() \n
 *   A addon can receive switches from the command line invocation
 *   of dlvhex. All command line parameters that are unknown to dlvhex will be
 *   passed to the addons, which can then check this list for their own
 *   switches.
 * .
 *
 *
 */
class DLVHEX_EXPORT AddonInterface
{
 protected:
...
public:
     ...
     */
    typedef std::map<std::string, boost::shared_ptr<AddonAtom> > AddonAtomFunctionMap;
     ...
     * AddonAtom objects. The strings denote the name of the atom as it should
     ...
     * {
     *     boost::shared_ptr<AddonAtom> newatom(new MyAtom);
     *     a["newatom"] = newatom;
     * }
     * \endcode
     *
     * Here, we assume to have defined an atom MyAtom derived from AddonAtom.
     ...
     *
     * \code
     * boost::shared_ptr<AddonAtom> split(new SplitAtom);
     * boost::shared_ptr<AddonAtom> concat(new ConcatAtom);
     * boost::shared_ptr<AddonAtom> substr(new SubstringAtom);
     * a["split"] = split;
     * a["concat"] = concat;
     * a["substr"] = substr;
     * \endcode
     ...
     */
    void
    setAddonName(const std::string& name)
    {
      this->addonName = name;
    }
